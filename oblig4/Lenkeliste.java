...
public class Lenkeliste<T> implements Liste<T> {
    ...
    private class LenketListeIterator implements Iterator<T> {
        ...
        public T next() {
            if (!hasNext()) {
                throw new NoSuchElementException("utenfor iterator");
            }
            if (hasNext()) {
                T data = node.data;
                node = node.neste;
        ...
        @Override
        public void remove() { throw new UnsupportedOperationException("Remove er ikke implementert");}
    }

    public Iterator<T> iterator() { return new LenketListeIterator(); }
}
